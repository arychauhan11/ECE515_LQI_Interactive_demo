...
md"""## Optimal Control of Inverted Pendulum: LQR and LQI

This project demonstrates the design and implementation of optimal feedback controllers for a challenging nonlinear system using both classical LQR (Linear Quadratic Regulator) and extended LQI (Linear Quadratic Integral) control strategies.
"""
...
The inverted pendulum is a canonical problem in control theory: a mass-spring system with a single point of contact, requiring continuous feedback to maintain the unstable upright configuration. This notebook explores:

1. **Symbolic Dynamics Derivation**: Using Lagrangian mechanics and automatic symbolic differentiation to obtain exact equations of motion
2. **System Linearization**: Creating a linear approximation valid near the equilibrium point
3. **Optimal Control Design**: Implementing LQR to minimize a quadratic performance criterion
4. **Extended Tracking Control**: Adding integral action via LQI for setpoint tracking and disturbance rejection
5. **Interactive Visualization**: Real-time comparison of linear and nonlinear system responses under parameter variation

All computations leverage Julia's ecosystem: `Symbolics.jl` for symbolic mathematics, `ControlSystems.jl` for control design, `DifferentialEquations.jl` for numerical integration, and `Pluto.jl` for interactive exploration.
"""
...
- **Linearization**: Understanding when the linear approximation is valid and its limitations
- **LQR Design**: How cost function weighting ($Q$ and $R$ matrices) shapes closed-loop behavior
- **Tracking vs. Regulation**: The difference between stabilization (LQR) and reference tracking (LQI)
- **Disturbance Rejection**: How integral action achieves steady-state accuracy under sustained disturbances
- **Trade-offs**: The inherent compromises between response speed, steady-state accuracy, and control effort
"""
...
In this section, we derive the system's equations of motion (EOM) using Lagrangian mechanics.
...
md"""
### Setting Up Symbolic Computation

We begin by declaring all symbolic variables needed to represent our system: parameters ($M, m, L, g$), states ($x, \theta$), and their temporal derivatives.
"""
...
Using Lagrangian mechanics, we compute the system dynamics from energy conservation. The Lagrangian is $\mathcal{L} = T - V$, where:

**Kinetic Energy:** The cart moves with velocity $\dot{x}$, while the pendulum bob has both horizontal and vertical motion components.
* $T_{cart} = \frac{1}{2} M (\dot{x})^2$
* $T_{bob} = \frac{1}{2} m [(\dot{x} + L\dot{\theta}\cos\theta)^2 + (L\dot{\theta}\sin\theta)^2]$

**Potential Energy:** Only gravitational potential energy varies as the pendulum swings.
* $V = m g L \cos(\theta)$

Applying the Euler-Lagrange equation $\frac{d}{dt}\frac{\partial \mathcal{L}}{\partial \dot{q}} - \frac{\partial \mathcal{L}}{\partial q} = F_q$ for each coordinate yields the nonlinear EOMs.
"""
...
md"""
### Extracting the Accelerations

The Euler-Lagrange equations are implicit in the accelerations. We solve these equations to obtain $\ddot{x}$ and $\ddot{\theta}$ as explicit functions of state and control input.
"""
...
md"""The resulting acceleration expressions can be verified (with some effort) by substituting back into the original Lagrange equations."""

# ╔═╡ 76c22e01-c3f8-4685-aca6-64386cfa1527
md"""
## (b) 🤖 Linearization Around the Unstable Equilibrium

Optimal control techniques like LQR work with linear models. We therefore linearize the nonlinear dynamics around the unstable equilibrium: pendulum upright, cart at origin, system at rest.

Using first-order Taylor expansion, the linear approximation near equilibrium is $\dot{\mathbf{z}} = A\mathbf{z} + B\mathbf{u}$, where $A$ and $B$ are Jacobian matrices evaluated at the operating point $\mathbf{z}_{eq} = [0, 0, 0, 0]^T$.
"""
...
md"""
## (c) ⚙️ Optimal Feedback Control via LQR

The Linear-Quadratic Regulator solves for the optimal feedback gain $K$ that minimizes

$$J = \int_0^{\infty} (\mathbf{z}^T Q \mathbf{z} + u^T R u) \, dt$$

This cost function balances two competing objectives: the $Q$ matrix penalizes state deviation (wanting fast stabilization), while the $R$ matrix penalizes control effort (wanting to minimize actuator usage). The relative magnitudes of $Q$ and $R$ define the control strategy: aggressive stabilization vs. energy-efficient damping.
"""
...
With symbolic derivations complete, we substitute realistic physical parameters and compute the linearized system matrices for numerical simulation.
"""
...
md"""
## (d) 🕹️ Interactive Design Space Exploration

The following sliders allow real-time parameter tuning and immediate feedback on system behavior. Adjusting these costs reveals the fundamental control design trade-offs:

 * **$Q_{11}$ (Position Penalty)**: Controls how aggressively the cart returns to setpoint
 * **$Q_{33}$ (Angle Penalty)**: Controls how forcefully the pendulum is stabilized upright
 * **$R$ (Control Cost)**: Higher values reduce actuator demand but slow response
 * **Initial Angle ($\theta_0$)**: Reveals how the linear approximation degrades for larger perturbations
 * **Position Setpoint ($x_{ref}$)**: The desired cart position for tracking

We compare the linear system response (assumed accurate near equilibrium) with actual nonlinear dynamics to demonstrate both the validity and limitations of linearization.
"""
...
begin
	...
	position_setpoint_slider = @bind position_setpoint_val Slider(-1.0:0.2:1.0, default=0.0, show_value=true);
end;

# ╔═╡ 7a2fc3bb-4211-408a-a326-c731deae316b
	begin
		# Build cost matrices from interactive parameters
		# Penalize position and angle deviation; velocity damping is implicit
		# All deviations measured relative to the setpoint in shifted coordinates
		Q = diagm([q1_val, 0, q3_val, 0])
		R = [r_val;;]

		# Solve the algebraic Riccati equation for optimal gain
		# K stabilizes shifted system: error in position = x - x_ref becomes new zero
...
With the LQR gain, we simulate both the linearized and nonlinear closed-loop systems under identical initial conditions and control law $u = -K\mathbf{z}$. The comparison demonstrates how accurately the linear model predicts actual behavior near the operating point.

Three key outputs are plotted: cart position, pendulum angle, and control force over 10 seconds. The **solid curve** represents actual nonlinear dynamics, while the **dashed curve** shows the linear approximation. Observe how:
- The controller's aggressiveness depends on $Q$ and $R$ weighting
- Linear and nonlinear predictions diverge as initial angle increases
- Energy trade-offs emerge as control cost is increased
"""


# ╔═╡ 86d18138-78b9-4bf1-bf21-739017de7fed
begin
	# Simulation configuration with position setpoint
	x₀ = 0  # Always start at 0
	θ₀ = theta_val
	# Shifted state coordinates: z_shifted = [x - x_ref, ẋ, θ, θ̇]
	# Initial condition in shifted coordinates
	z₀ = [x₀ - position_setpoint_val, 0.0, θ₀, 0.0]
	t_span = (0.0, 10.0)

	# Prepare nonlinear dynamics function
	...
	# Nonlinear closed-loop dynamics with LQR feedback (in shifted coordinates)
	function nonlinear_dynamics!(dz, z, p, t)
		M_val, m_val, L_val, g_val, K_mat, x_ref = p
		# z is in shifted coordinates: [x - x_ref, ẋ, θ, θ̇]
		# Reconstruct absolute position for dynamics computation
		x_abs = z[1] + x_ref
		z_abs = [x_abs, z[2], z[3], z[4]]
		
		u_val = (-K_mat * z)[1]
		dz_abs = f_nonlinear_oop(z_abs, u_val, [M_val, m_val, L_val, g_val], t)
		
		# Return derivatives in shifted coordinates (same as absolute since x_ref is constant)
		dz .= dz_abs
	end
	
	# Linear closed-loop dynamics: dz/dt = (A - BK)z (already in shifted coordinates)
	...
	# Solve both systems
	p_nonlin = (params[M], params[m], params[L], params[g], K, position_setpoint_val)
...
begin
	# Plot 1: Cart Position (x) - convert shifted coordinates to absolute
	p1 = plot(
		sol_lin.t, [s[1] + position_setpoint_val for s in sol_lin.u], 
		label="Linear", linestyle=:dash, lw=2,
		title="Cart Position", xlabel="Time (s)", ylabel="Position (m)"
	)
	plot!(
		p1, sol_nonlin.t, [s[1] + position_setpoint_val for s in sol_nonlin.u],
		label="Nonlinear", lw=2
	)
	hline!(p1, [position_setpoint_val], label="Setpoint", linestyle=:dot, lw=2, color=:gray)
...
**Cost Weighting Effects:**
- **Large $Q_{33}$**: Aggressive angle correction → higher control effort and potential cart overshoot as the pendulum "whips" to vertical
- **Large $Q_{11}$**: Prioritizes cart centering → smoother position convergence at the cost of slower angle stabilization
- **Large $R$**: Favors smooth, energy-efficient control → sluggish response; excessive values prevent stabilization

**Linearization Validity:**
- For small initial angles ($\theta_0 < 0.3$ rad), linear and nonlinear responses are nearly identical
- As $\theta_0$ increases, nonlinear effects (especially gravity) cause prediction errors
- This motivates use of gain-scheduling or nonlinear control for larger operating regions
"""

# ╔═╡ c3d4e5f6-a7b8-49ca-91bd-2e3f4a5b6c7d
md"""
## (e) 🎯 LQI Controller Implementation

The Linear-Quadratic Integral (LQI) controller extends the LQR framework to handle tracking and disturbance rejection. Unlike LQR which only stabilizes to the origin, LQI can track a reference **cart position** setpoint and reject constant disturbances.

### Augmented State-Space Model

To implement LQI, we augment the system with an integral error state:

$$\frac{d}{dt}\begin{bmatrix} \mathbf{z} \\ z_i \end{bmatrix} = \begin{bmatrix} A & 0 \\ -C & 0 \end{bmatrix}\begin{bmatrix} \mathbf{z} \\ z_i \end{bmatrix} + \begin{bmatrix} B \\ 0 \end{bmatrix}\mathbf{u} + \begin{bmatrix} B \\ 0 \end{bmatrix}\mathbf{d}$$

where:
- $z_i = \int (x_{ref} - x) dt$ is the integral of cart position error
- $C = [1, 0, 0, 0]$ selects cart position for tracking
- $\mathbf{d}$ is the constant disturbance
- Control law: $\mathbf{u} = -[K_p | K_i]\begin{bmatrix} \mathbf{z} \\ z_i \end{bmatrix}$
"""

# ╔═╡ d4e5f6a7-b8c9-40db-92ce-3f4a5b6c7d8e
begin
	# Add slider for LQI disturbance
	disturbance_slider = @bind disturbance_val Slider(-5.0:1.0:5.0, default=0.0, show_value=true);
end;

# ╔═╡ e5f6a7b8-c9da-41ec-93df-4a5b6c7d8e9f
md""" Disturbance slider (constant force): $(disturbance_slider)"""

# ╔═╡ a81790e2-d54b-11f0-8163-c3765dfad0e6
md""" $x_{ref}$ slider (position setpoint): $(position_setpoint_slider)"""

# ╔═╡ d7a8b9ca-ebfc-43be-95f1-6c7d8e9fa0b1
begin
	# ========== LQI Setup ==========
	# Augmented state in shifted coordinates: [x - x_ref, ẋ, θ, θ̇, x_i]
	# where x_i = ∫(0 - (x - x_ref))dt = -∫(x - x_ref)dt accumulates error in shifted position
	# Track shifted position (which should go to zero) or equivalently track x_ref in absolute coordinates
	C_track = [1 0 0 0]  # Select position error (in shifted coordinates) for tracking
	
	# Augmented system matrices
	A_aug_lqi = [A_num zeros(4, 1); -C_track zeros(1, 1)]
	B_aug_lqi = [B_num; zeros(1, 1)]
	
	# Create augmented state-space model (5x5)
	sys_lqi_aug = ss(A_aug_lqi, B_aug_lqi, I(5), 0)
	
	# LQI cost function weights
	Q_lqi = diagm([q1_val, 0, q3_val, 0, q1_val])  # Penalize position error and integral error equally
	R_lqi = [r_val;;]

	# Compute the LQI gain K (1x5 matrix)
	K_lqi = lqr(sys_lqi_aug, Q_lqi, R_lqi)
end;

# ╔═╡ b8c9d0e1-acbe-45af-97a3-8e9fa0b1c2d3
begin
	# ========== LQR with Disturbance ==========
	# Dynamics work in shifted coordinates: z = [x - x_ref, ẋ, θ, θ̇]
	function nonlinear_dynamics_lqr_dist!(dz, z, p, t)
		M_val, m_val, L_val, g_val, K_mat, d_dist, x_ref = p
		# Reconstruct absolute position for nonlinear dynamics
		x_abs = z[1] + x_ref
		z_abs = [x_abs, z[2], z[3], z[4]]
		
		u_val = (-K_mat * z)[1] + d_dist
		dz_abs = f_nonlinear_oop(z_abs, u_val, [M_val, m_val, L_val, g_val], t)
		
		# Return in shifted coordinates
		dz .= dz_abs
	end
	
	function linear_dynamics_lqr_dist!(dz, z, p, t)
		A_mat, B_mat, K_mat, d_dist = p
		u_val = (-K_mat * z)[1] + d_dist
		dz .= A_mat * z + B_mat * [u_val]
	end
	
	# ========== LQI with Disturbance and Setpoint ==========
	# Working in shifted coordinates where the setpoint becomes the origin
	function nonlinear_dynamics_lqi_dist!(dz, z, p, t)
		M_val, m_val, L_val, g_val, K_mat, d_dist, x_ref = p
		
		# Extract 4-state and integral state (all in shifted coordinates)
		z_state = z[1:4]  # [x - x_ref, ẋ, θ, θ̇]
		z_i = z[5]
		
		# Reconstruct absolute position for nonlinear dynamics
		x_abs = z_state[1] + x_ref
		z_abs = [x_abs, z_state[2], z_state[3], z_state[4]]
		
		# Integral error in shifted coordinates: dx_i/dt = -z_state[1] (negative of position error)
		dz_i = -z_state[1]
		
		# Control input: u = -K*[z_shifted; z_i] + disturbance
		augmented_state = [z_state; z_i]
		u_val = (-K_mat * augmented_state)[1] + d_dist
		
		# Call nonlinear dynamics
		dz_abs = f_nonlinear_oop(z_abs, u_val, [M_val, m_val, L_val, g_val], t)
	...
	function linear_dynamics_lqi_dist!(dz, z, p, t)
		A_aug_mat, B_aug_mat, K_mat, d_dist = p
		
		# z in shifted coordinates: [x - x_ref, ẋ, θ, θ̇, x_i]
		augmented_state = z
		u_val = (-K_mat * augmented_state)[1] + d_dist
		
		dz .= A_aug_mat * z + B_aug_mat * [u_val]
	end
	
	# ========== Run Simulations ==========
	# Initial conditions (in shifted coordinates)
	z₀_lqr = [0.0, 0.0, theta_val, 0.0]  # [x - x_ref, ẋ, θ, θ̇]
	z₀_lqi = [0.0, 0.0, theta_val, 0.0, 0.0]  # [x - x_ref, ẋ, θ, θ̇, x_i]
	
	# LQR with disturbance - separate parameter tuples for linear and nonlinear
	p_lqr_nonlin = (params[M], params[m], params[L], params[g], K, disturbance_val, position_setpoint_val)
	p_lqr_lin = (A_num, B_num, K, disturbance_val)
	
	prob_lqr_lin = ODEProblem(linear_dynamics_lqr_dist!, z₀_lqr, t_span, p_lqr_lin)
	sol_lqr_lin = solve(prob_lqr_lin, Tsit5(), saveat=0.05)
	
	prob_lqr_nonlin = ODEProblem(nonlinear_dynamics_lqr_dist!, z₀_lqr, t_span, p_lqr_nonlin)
	sol_lqr_nonlin = solve(prob_lqr_nonlin, Tsit5(), saveat=0.05)
	
	# LQI with disturbance and setpoint - separate parameter tuples for linear and nonlinear
	p_lqi_nonlin = (params[M], params[m], params[L], params[g], K_lqi, disturbance_val, position_setpoint_val)
	p_lqi_lin = (A_aug_lqi, B_aug_lqi, K_lqi, disturbance_val)
	
	prob_lqi_lin = ODEProblem(linear_dynamics_lqi_dist!, z₀_lqi, t_span, p_lqi_lin)
	sol_lqi_lin = solve(prob_lqi_lin, Tsit5(), saveat=0.05)
	
	prob_lqi_nonlin = ODEProblem(nonlinear_dynamics_lqi_dist!, z₀_lqi, t_span, p_lqi_nonlin)
	sol_lqi_nonlin = solve(prob_lqi_nonlin, Tsit5(), saveat=0.05)
end;

# ╔═╡ c9d0e1f2-cfde-46bf-98b4-9fa0b1c2d3e4
md"""
### 📊 LQR vs LQI Comparison

The plots below compare LQR (purple) with LQI (orange). Notice:
- **LQR**: Stabilizes to origin, doesn't track setpoint, affected by constant disturbance
- **LQI**: Tracks the position setpoint and rejects constant disturbances (in steady state)
"""

# ╔═╡ f0d1e2f3-cdef-47cd-99c5-0ab1f1d2e3f4
begin
	# Plot 1: Cart Position
	p1_comp = plot(
		sol_lqr_lin.t, [s[1] + position_setpoint_val for s in sol_lqr_lin.u],
		label="LQR Linear", linestyle=:dash, lw=2, color=:purple,
		title="Cart Position: LQR vs LQI", xlabel="Time (s)", ylabel="Position (m)"
	)
	plot!(p1_comp, sol_lqr_nonlin.t, [s[1] + position_setpoint_val for s in sol_lqr_nonlin.u],
		label="LQR Nonlinear", lw=2, color=:purple, alpha=0.5)
	plot!(p1_comp, sol_lqi_lin.t, [s[1] + position_setpoint_val for s in sol_lqi_lin.u],
		label="LQI Linear", linestyle=:dash, lw=2, color=:orange)
	plot!(p1_comp, sol_lqi_nonlin.t, [s[1] + position_setpoint_val for s in sol_lqi_nonlin.u],
		label="LQI Nonlinear", lw=2, color=:orange, alpha=0.5)
	hline!(p1_comp, [position_setpoint_val], label="Setpoint", linestyle=:dot, lw=2, color=:black)
	
	# Plot 2: Pendulum Angle
	p2_comp = plot(
		sol_lqr_lin.t, [rad2deg(s[3]) for s in sol_lqr_lin.u],
		label="LQR Linear", linestyle=:dash, lw=2, color=:purple,
		title="Pendulum Angle", xlabel="Time (s)", ylabel="Angle (°)"
	)
	plot!(p2_comp, sol_lqr_nonlin.t, [rad2deg(s[3]) for s in sol_lqr_nonlin.u],
		label="LQR Nonlinear", lw=2, color=:purple, alpha=0.5)
	plot!(p2_comp, sol_lqi_lin.t, [rad2deg(s[3]) for s in sol_lqi_lin.u],
		label="LQI Linear", linestyle=:dash, lw=2, color=:orange)
	plot!(p2_comp, sol_lqi_nonlin.t, [rad2deg(s[3]) for s in sol_lqi_nonlin.u],
		label="LQI Nonlinear", lw=2, color=:orange, alpha=0.5)
	
	# Plot 3: Control Input
	u_lqr_lin = [-(K * s)[1] for s in sol_lqr_lin.u]
	u_lqr_nonlin = [-(K * s)[1] for s in sol_lqr_nonlin.u]
	u_lqi_lin = [-(K_lqi * s)[1] for s in sol_lqi_lin.u]
	u_lqi_nonlin = [-(K_lqi * s)[1] for s in sol_lqi_nonlin.u]
	
	p3_comp = plot(
		sol_lqr_lin.t, u_lqr_lin .+ disturbance_val,
		label="LQR Linear", linestyle=:dash, lw=2, color=:purple,
		title="Control Input (including disturbance)", xlabel="Time (s)", ylabel="Force (N)"
	)
	plot!(p3_comp, sol_lqr_nonlin.t, u_lqr_nonlin .+ disturbance_val,
		label="LQR Nonlinear", lw=2, color=:purple, alpha=0.5)
	plot!(p3_comp, sol_lqi_lin.t, u_lqi_lin .+ disturbance_val,
		label="LQI Linear", linestyle=:dash, lw=2, color=:orange)
	plot!(p3_comp, sol_lqi_nonlin.t, u_lqi_nonlin .+ disturbance_val,
		label="LQI Nonlinear", lw=2, color=:orange, alpha=0.5)
	
	plot(p1_comp, p2_comp, p3_comp, layout=(3,1), size=(900,800), legend=:topright, link=:x)
end
...
**System Design Note:**
Both controllers are designed using linearization around the origin ($\theta = 0$, $x = 0$, system at rest). While the LQI controller tracks a non-zero setpoint in the shifted coordinate system, the underlying linear approximation is still valid only near the equilibrium point at zero.

**Key Differences:**
- **LQR**: Regulator - stabilizes system to equilibrium at origin. Cannot track references or reject steady-state disturbances. Without disturbances, can be highly effective.
- **LQI**: Integral controller - tracks reference setpoints and rejects constant disturbances perfectly in steady state using integral action.

**When Each Shines:**
- **Without disturbance**: LQR often performs comparably or even better than LQI, as it has one fewer state to manage and can focus purely on stabilization and setpoint tracking through proper initial conditions.
- **With disturbance**: LQI's integral action is essential—it automatically compensates for sustained disturbances and maintains steady-state accuracy. LQR will exhibit a permanent offset proportional to the disturbance magnitude.

**What to Explore:**
- Increase disturbance: See how LQR is offset but LQI compensates
- Zero disturbance: Compare response speeds—LQR may reach the setpoint faster
- Increase Q₁₁: Both become more aggressive in position control
- Increase R: Both become "lazy", LQI may struggle with disturbance rejection
"""
...
manifest_format = "2.0"
project_hash = "02cc18a4e7f4123bd2eeb73ff4a4dc1eac62a72f"
